...
app.get('/', requireAuth, async (req, res) => {
  try {
    // Pick 2 different random categories and random pages for variety each load
    ...
    // Combine, deduplicate by movie ID, then shuffle
    const seen = new Set();
    const combined = [...(d1.results || []), ...(d2.results || [])].filter(m => {
